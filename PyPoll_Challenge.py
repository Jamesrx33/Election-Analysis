...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in county_list:
        ...
        county_summary = (
            f"{county_name}: {percentage:.1f}% ({countyVotes:,})\n")
    ...
    largest_countyResult = (
            f"\n----------------------\nLargest County Turnout: {largest_countyName} ({largest_count:,})\n----------------------\n")
    print(largest_countyResult)

    # 8: Save the county with the largest turnout to a text file.
    txt_file.write(largest_countyResult)

    # Save the final candidate vote count to the text file.
    for candidate_name in candidate_votes:

        # Retrieve vote count and percentage
        candidateVotes = candidate_votes.get(candidate_name)
        vote_percentage = float(candidateVotes) / float(total_votes) * 100
        candidate_results = (
            f"{candidate_name}: {vote_percentage:.1f}% ({candidateVotes:,})\n")
